...
class DatabaseManager:
    ...
    def check_datacache_table(self):
        current_datetime = datetime.now()
        current_date = current_datetime.date() - timedelta(hours=24)
        conn = sqlite3.connect(db_name)
        cursor = conn.cursor()
        query = f"SELECT count(*) as result FROM datacache_report WHERE start_timestamp LIKE '{current_date}%'"
        cursor.execute(query)
        result_count = cursor.fetchall()[0]  # Directly get the count from the first row
        cursor.close()
        return result_count
